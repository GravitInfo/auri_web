...
const calculateDuration = (from, to) => {
  ...
  if (hours < 0) hours += 24; // handle overnight times
...
const orgServicesController = {
  ...
  create: async (req, res) => {
    try {
      ...
      // Auto-calculate duration if timing provided
      if (req.body.timing_from && req.body.timing_to) {
        duration = calculateDuration(req.body.timing_from, req.body.timing_to);
      }

      // ✅ fix: handle auto category icon or uploaded icon
      const iconName = req.file
        ? req.file.filename
        : req.body.existing_icon || null;
  ...
  update: async (req, res) => {
    try {
      ...
      // ✅ fix: handle existing icon if not uploading new one
      const iconName = req.file
        ? req.file.filename
        : req.body.existing_icon || req.body.icon || null;
